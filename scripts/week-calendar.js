...
import { isEventAllDay, eventsStartBefore, eventsEndBefore, initDynamicEvent, eventCollidesWith, adjustDynamicEventMaxLines } from "./event.js";
...
function initColumn(parent, weekDay, events) {
    ...
    for (const eventsWithDynamicStyle of eventsWithDynamicStyles) {
        initDynamicEvent(
            calendarColumnElement,
            eventsWithDynamicStyle.event,
            eventsWithDynamicStyle.styles
        )
    }
...
function calculateEventsDynamicStyles(events) {
    ...
    const columnWith = 100 / totalColumns;
    ...
    return initialEventGroupItems.map((eventGroupItem) => {
        ...
        const leftPercentage = columnWith * eventGroupItem.columnIndex;
        const rightPercentage = columnWith * ((totalColumns - eventGroupItem.columnIndex) - eventGroupItem.columnSpan);
...
function sortEventsByTime(events) {
    events.sort((eventA, eventB) => {
        if (eventsStartBefore(eventA, eventB)) {
            return -1;
        }

        if (eventsStartBefore(eventB, eventA)) {
            return 1;
        }

        return eventsEndBefore(eventA, eventB) ? 1 : -1;
    });
}
