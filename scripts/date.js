...
export function generateMonthCalendarDays(currentDate) {
    ...
    const lastDayOfPreviousMonthWeeDay = lastDayOfPreviousMonthDate.getDay();

    if (lastDayOfPreviousMonthWeeDay !== 6) {
        for (let i = lastDayOfPreviousMonthWeeDay; i >= 0; i--) {
    ...
    for (let i = 1; i <= lastDayOfCurrentMonthDate.getDate(); i++) {
    ...
    for (let i = 1; i <= missingDayAmount; i++) {
